...
router.post('/', (req, res, next) => {
  ...
  let time = Date.now();//创建服务器上传时间
   ...
   req.files && req.files.forEach((file, index) => {
     ...
     if (file.fieldname === 'home') {
       home = require('../../config/path').home.uploadUrl + file.filename + pathLib.parse(file.originalname).ext;
     }
  ...
  //未传图片处理
  if (!home) home = require('../../config/path').normal;
      ...
      collection.insertOne(
        ...
        (err, result) => {
          if (!err && result.result.ok) {
            ...
            res.send({ error: 0, mess: '成功', data: { id: result.insertedId, title, des, time, detail: { icon, auth, content } } })
      ...
      _id: ObjectId(id)
        ...
        mgdb({
          ...
          collection.updateMany({
            ...
            $set:{
              title,des,home,time,detail:{icon,auth,content}
            }
          },{
            upsert:false, //插入
